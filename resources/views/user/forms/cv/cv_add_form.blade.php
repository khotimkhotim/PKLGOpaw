<form class="form" id="add_edit_profile_cv" method="POST" action="{{ route('store.profile.cv', [$user->id]) }}" target="cv_iframe">{{ csrf_field() }}
      <div class="modal-header">
        ...
        <h4 class="modal-title">{{__('Tambah Proposal')}}</h4>
      </div>
<div class="modal-body">
        <div class="form-body">
          <div class="form-group" id="div_name">
            <label for="name" class="bold">{{__('Judul Proposal')}}</label>
          ...
          <div class="form-group" id="div_cv_file">
            <label for="cv_file" class="bold">{{__('File Proposal')}}</label>
      ...
      <div class="modal-footer">
        <button type="button" class="btn dark btn-outline" data-dismiss="modal">{{__('Tutup')}}</button>
        <button type="submit" class="btn btn-large btn-primary">{{__('Tambah Proposal')}} <i class="fa fa-arrow-circle-right" aria-hidden="true"></i></button>
      </div>
      </form>
